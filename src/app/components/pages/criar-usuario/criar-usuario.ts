...
mostrarConfirmarSenha: any;
 ...
 //limpar mensagens
    ...
    if (this.formCriarUsuario.value.senha !== 
             this.formCriarUsuario.value.confirmarSenha) {
    ...
    this.http.post(environment.apiUsuarios + '/criar', 
                     this.formCriarUsuario.value)
      .subscribe({
        next: (resposta : any) => { //captura a resposta de sucesso
          this.mensagemSucesso.set(`Parabéns ${resposta.nome}, 
